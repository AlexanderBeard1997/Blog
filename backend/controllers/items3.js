...
const userObj = new Comment({
  comment, blogId
    
})
    // await for it to be saved
    const newComment = await userObj.save()
    // respond with json()
...
const editComment = async (req, res) => {
    ...
    let comment = await Comment.findByIdAndUpdate(id, { comment: req.body.comment})
...
const deleteComment = async (req, res) => {
   ...
   let user = await Comment.findByIdAndDelete(id)
